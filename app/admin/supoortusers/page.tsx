...
import { FetchallUser, GetallBookings, PatchUser, registerUser } from "@/lib/api"
...
const [newUser, setNewUser] = useState({
  ...
  const fetchUsers = async () => {
    ...
    setUsers(response.filter((user: { role: string }) => user.role === "support"))
  }
  useEffect(() => {
    fetchUsers()
  }, []) // Add empty dependency array
...
const toggleUserStatus = async (userId: number, currentStatus: boolean) => {
  ...
  try {
    ...
    const response = await PatchUser(String(userId), payload); // <- your API function
    ...
    // ✅ Update selectedUser in-place if it matches
    ...
    // Optionally: if you're using `fetchUsers()` elsewhere
    // fetchUsers?.();
  } catch (error) {
...
const handleCreateSupportUser = async () => {
  ...
  try {
    await registerUser(name, phone, email, password,  Role.SUPPORT,"approved");

    toast.success("Support user created successfully.");

    // Reset form, close modal, and reload list
    setNewUser({ name: "", email: "", password: "", phone: "", role: "support" });
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 tracking-tight">Flight Management </h1>
          <p className="text-gray-600 mt-2 text-lg">Manage Flight Manager on your platform</p>
        </div>
...
>
  ...
  <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl">
    <DialogHeader>
      <DialogTitle className="text-2xl font-bold text-gray-900">
        Support User - {selectedUser?.name}
      </DialogTitle>
      <DialogDescription className="text-gray-600">
        View detailed information of support user
      </DialogDescription>
    </DialogHeader>
  ...
  <DialogContent className="max-w-lg rounded-2xl">
    <DialogHeader>
      <DialogTitle className="text-2xl font-bold text-gray-900">Create Support User</DialogTitle>
      <DialogDescription className="text-gray-600">
        Fill in the details to add a new support user
      </DialogDescription>
    </DialogHeader>
